...
export default function Home({data}) {
  ...
  const youtubeBtn = () => {
    if (meal.strYoutube)  {
      return (
        <button id="btn" style={{display:expanded? 'grid' : 'none'}} className="bg-secondary text-lg font-bold py-1 px-4 flex-grow  grid place-content-center rounded-full text-white active:drop-shadow-none active:translate-x-1 active:translate-y-1 transition-all shadow-xl">
          <a href={meal.strYoutube} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 w-fit text-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-8 h-8">
  ...
  return (
    <div className="bg-primary">
      ...
      <div className="bg-primary min-h-[100dvh] flex flex-col gap-20 lg:justify-between" >
        ...
        <div className=" relative flex aspect-square max-h-[50dvh] shadow-xl">
          <img src={meal.strMealThumb} className="rounded-xl  w-full  object-cover"/>
        </div>
         <div className="bg-white rounded-xl p-12 pb-4 mb-40 shadow-xl">
            <div  className="flex flex-col gap-4">
              ...
              <div className="flex justify-end gap-2 flex-col-reverse lg:flex-row">
                {youtubeBtn()}
                <button title="click to show recipe" onClick={handleClick} className="flex ml-auto shadow-xl  w-full justify-center font-bold items-center hover:cursor-pointer gap-2 text-white rounded-full py-1 pl-4 pr-2 hover:pr-4 bg-secondary  text-lg lg:w-fit lg:text-[0px] hover:text-lg transition-all">
        ...
        <button onClick={refresh} title="Roll the dice" className="fixed bottom-4 shadow-xl hover:shadow-none right-4 active:rotate-[360deg] delay-rotate-100 hover:drop-shadow-none hover:translate-x-1 hover:translate-y-1 transition-all hover:shadow-none w-fit bg-secondary grid place-content-center p-4 rounded-full ">
        <Image src={dice} width={48} height={48} className="invert"/>
      </button>

        <footer className="fixed bottom-4 left-4 font-bold" >
            <a href="https://github.com/Andreas-Espelund" target="_blank" rel="noopener noreferrer" className="flex shadow-xl justify-center items-center hover:cursor-pointer gap-2 rounded-full w-fit py-1 pl-4 pr-2 hover:pr-4 bg-accent text-white text-[0px] hover:text-lg transition-all"> 
